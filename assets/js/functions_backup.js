...
$(function () {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data !== null) {
					...
					$("#editServiceAreaNewProvince").val(data.province).trigger("change");

					// append city & district pake timeout
					setTimeout(function() {
			            $("#editServiceAreaNewCity").val(arrCity).trigger("change");
			            // Jeda deui 500ms saatos Kota di-set, nembé Kacamatan dieusian
			            setTimeout(function() {
			                $("#editServiceAreaNewDistrict").val(arrDistrict).trigger("change");
			            }, 300);
			        }, 300);
				}
			}
	...
	$(document).on("change", 'input[name="editServiceAreaNewDistrict[]"]', function() {
	    ...
	    // 2. NGUMPULKEUN SAKABEH KECAMATAN NU DICEKLIS (DINA BENTUK ARRAY)
	    var districtsArray = [];
	    ...
	    // Kosongkeun container desa heula unggal aya parobahan ceklis kecamatan
	    $('#editServiceAreaNewSubdistrictContainer').val(null).trigger('change').html('');
	    
	    // 3. CLERICAL CHECK: Lamun euweuh kecamatan nu diceklis pisan, ulah jalankeun AJAX
	    if (districtsArray.length === 0) {
	        return;
	    }

	    // 4. JALANKEUN AJAX GET DESA
	    $.ajax({
